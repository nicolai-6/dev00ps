...
function constants() {
    ...
    X509_SUBJECT='/C=DE/O=dev0Ops/OU=DevOps/CN=dev0Ops Sub CA Client - 1'
    CA_SERIAL=./db/root-ca.serial
}

#### sub ca key ####
## create sub ca client key and encrypt it - will prompt for passphrase
function create_key() {
    openssl genpkey \
    -aes256 \
    -algorithm EC \
